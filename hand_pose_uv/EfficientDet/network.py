...
from utils.help_functions import plot_predicted_heatmaps
...
def liftpose(uv_coords, features):
    ...
    xyz = layers.Dense(63, activation='linear', name = 'xyz')(added)
...
def calc_xyz(z_can, uv_coords):
    ...
    xy = uv_coords * z_can
    xy_zcan = layers.Concatenate(axis = -1, name = 'xyz')([xy,z_can])
    return xy_zcan
...
def efficientdet(phi,input_shape = (224,224,3),include_depth = False, num_classes=20, weighted_bifpn=False, freeze_bn=False, score_threshold=0.01, train_full = True):
    ...
    feature2 = layers.Conv2D(21, kernel_size = 1, strides = 1, padding = "same", activation = 'linear')(feature2_cont)
